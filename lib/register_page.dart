    ...
    return TextFormField(
      ...
      validator: (value) {
        ...
        if (value != null && value.trim().isNotEmpty) {
          ...
          if (isName) {
            if (!RegExp(r"^[a-zA-Z\s'-]+$").hasMatch(trimmedValue)) return 'Only letters allowed';
            if (trimmedValue.length < 2) return 'Must be at least 2 characters';
          }
          if (isEmail) {
            // Updated email regex to ensure a valid domain (e.g., @gmail.com, @yahoo.com)
            final emailRegex = RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$');
            if (!emailRegex.hasMatch(trimmedValue)) return 'Enter a valid email (e.g. name@gmail.com)';
          }
          if (isPhone) {
            if (trimmedValue.length != 11) return 'Must be 11 digits';
            if (!trimmedValue.startsWith('09')) return 'Must start with 09';
          }
          if (isPassword) {
            if (trimmedValue.length < 8) return 'Password must be at least 8 characters';
            if (!RegExp(r'[A-Z]').hasMatch(trimmedValue)) return 'Add at least one uppercase letter';
            if (!RegExp(r'[a-z]').hasMatch(trimmedValue)) return 'Add at least one lowercase letter';
            if (!RegExp(r'[0-9]').hasMatch(trimmedValue)) return 'Add at least one number';
            if (!RegExp(r'[!@#$%^&*(),.?":{}|<>]').hasMatch(trimmedValue)) return 'Add at least one special character';
          }
          if (isConfirm && trimmedValue != _passwordController.text.trim()) return 'Passwords do not match';
        }
        return null;
      },
    );
